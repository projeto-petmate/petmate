import './ModalFecharCarrinho.css';
import { IoLocationSharp } from "react-icons/io5";
import { CgCloseO } from 'react-icons/cg';
import { useState, useEffect } from 'react';
import { finalizarCarrinho, getPedidosItensUsuarioLogado } from '../apiService';
import Swal from 'sweetalert2';
import InputMask from 'react-input-mask'
import { FaBroom } from 'react-icons/fa6';
import { FaSearch } from 'react-icons/fa';

function ModalFecharCarrinho({
  isOpen,
  setIsOpen,
  carrinhoAtual,
  itens,
  ...
  const [buscandoPedidoAnterior, setBuscandoPedidoAnterior] = useState(false);
  const [pedidoAnteriorEncontrado, setPedidoAnteriorEncontrado] = useState(false);
 ...
 const buscarUltimoPedidoUsuario = async () => {
  if (!userLogado?.id_usuario) {
    console.log('❌ Usuário não logado ou ID não disponível');
    return;
  }

  try {
    setBuscandoPedidoAnterior(true);
    console.log(`🔍 Buscando último pedido do usuário ${userLogado.id_usuario}...`);

    const response = await getPedidosItensUsuarioLogado(userLogado, null, 1);

    if (response.itens_por_pedido && response.itens_por_pedido.length > 0) {
      const ultimoPedido = response.itens_por_pedido[0];
      const endereco = ultimoPedido.endereco_entrega; 
      ...
      if (endereco) {
        if (endereco.cep) {
          setCep(endereco.cep.replace(/(\d{5})(\d{3})/, '$1-$2'));
        }
        if (endereco.uf) setEstado(endereco.uf);
        ...
        if (endereco.nome_destinatario) setNomeDestinatario(endereco.nome_destinatario);
        if (endereco.numero_residencia) setNumeroResidencia(endereco.numero_residencia);
        if (endereco.complemento) setComplemento(endereco.complemento);
        if (endereco.telefone_contato) {
          const telefoneFormatado = endereco.telefone_contato.replace(/(\d{2})(\d{5})(\d{4})/, '($1) $2-$3');
          setTelefone(telefoneFormatado);
        }

        setPedidoAnteriorEncontrado(true);
        setCepValido(true); 
      ...
      console.log('Nenhum pedido anterior encontrado para este usuário');
      setPedidoAnteriorEncontrado(false);
    }

  } catch (error) {
    console.error('Erro ao buscar pedido anterior:', error);
    setPedidoAnteriorEncontrado(false);
  } finally {
    setBuscandoPedidoAnterior(false);
  }
};
  ...
  const limparFormulario = () => {
    setCep('');
    setEstado(userLogado.uf || '');
    setCidade(userLogado.cidade || '');
    setBairro(userLogado.bairro || '');
    setLogradouro('');
    setNomeDestinatario('');
    setNumeroResidencia('');
    setComplemento('');
  ...
  useEffect(() => {
    if (isOpen && userLogado?.id_usuario) {
  ...
  return (
    <div className="overlay-entrega">
      <div className="container-entrega">
        ...
        <div className="status-preenchimento">
          {buscandoPedidoAnterior && (
            <div className="status-buscando">
              <span className="spinner-tiny"></span>
              Buscando dados do último pedido...
            </div>
          )}
          {pedidoAnteriorEncontrado && (
            <div className="status-encontrado">
              Dados preenchidos automaticamente
              <button
                type="button"
                className="botao-limpar-form"
          ...
          {!buscandoPedidoAnterior && !pedidoAnteriorEncontrado && (
            <div className="status-manual">
               Preenchendo os dados manualmente
              <button
                type="button"
                className="botao-buscar-anterior"
                onClick={buscarUltimoPedidoUsuario}
                title="Tentar buscar pedido anterior novamente"
              >
...
export default ModalFecharCarrinho