...
const swaggerOptions = {
  definition: {
    ...
    info: {
      ...
      version: "1.0.0",
      description:
        "Documentação da API para gerenciar pets, usuários, ONGs e comentários.",
    },
  },
...
app.get("/pets/:id", async (req, res) => {
  const { id } = req.params;
  try {
    const result = await pool.query("SELECT * FROM pets WHERE id_pet = $1", [
      id,
    ]);
    if (result.rows.length === 0) {
...
app.post("/pets", async (req, res) => {
  ...
  if (!id_usuario && !id_ong) {
    return res
      .status(400)
      .json({ error: "É necessário informar id_usuario ou id_ong" });
  }
  ...
  if (id_usuario && id_ong) {
    return res
      ...
      .json({
        error:
          "Um pet não pode pertencer a um usuário e a uma ONG ao mesmo tempo",
      });
  }
...
app.delete("/pets/:id", async (req, res) => {
  ...
  try {
    const result = await pool.query(
      "DELETE FROM pets WHERE id_pet = $1 RETURNING *",
      [id]
    );
    if (result.rows.length === 0) {
      return res.status(404).json({ error: "Pet não encontrado" });
...
app.post("/comentarios", async (req, res) => {
  ...
  if (!texto || (!id_usuario && !id_ong)) {
    return res
      .status(400)
      .json({ error: "Texto e ID de usuário ou ONG são obrigatórios." });
  }
  ...
  try {
    ...
    if (id_usuario) {
      ...
      const ongResult = await pool.query(
        "SELECT nome_ong, foto_perfil FROM ongs WHERE id_ong = $1",
        [id_ong]
      );
...
app.post("/denuncias", async (req, res) => {
  ...
  if (!motivo || !tipo_objeto || !id_objeto || !tipo_denunciante) {
    return res
      ...
      .json({
        error:
          "Motivo, tipo do objeto, ID do objeto e tipo do denunciante são obrigatórios.",
      });
  }

  //Validação: Apenas um tipo de denunciante deve ser fornecido
  if (
    (tipo_denunciante === "usuario" &&
      (!id_denunciante || id_ong_denunciante)) ||
    (tipo_denunciante === "ong" && (!id_ong_denunciante || id_denunciante))
  ) {
    return res
      .status(400)
      .json({
        error: "Forneça apenas o ID correspondente ao tipo do denunciante.",
      });
  }
...
app.put("/denuncias/:id", async (req, res) => {
  ...
  if (!["pendente", "em análise", "resolvido"].includes(status)) {
    return res
      .status(400)
      .json({
        error: 'Status inválido. Use "pendente", "em análise" ou "resolvido".',
      });
  }
...
app.delete("/denuncias/:id", async (req, res) => {
  ...
  try {
    const result = await pool.query(
      "DELETE FROM denuncias WHERE id_denuncia = $1 RETURNING *",
      [id]
    );
    if (result.rows.length === 0) {
      return res.status(404).json({ error: "Denúncia não encontrada" });
